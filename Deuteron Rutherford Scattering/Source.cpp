/*
12C + 12C Background Simulator

This program is intended to output the expected count rates and energies for the background protons generated in the 12C fusion experiment. 
There are two main parts - the simpler elastic collision case, as modelled by Rutherford scattering, and the more complex inelastic fusion case. 

The modules are organized in the following order:

1. A method to determine the final velocity of a particle after a collision with a static target particle.

2. A method to determine the angular Rutherford cross-section of the ejectile particle with a target particle. This method relies on the first for the energy of the ejectile.

3. A numerical integration method to determine the total Rutherford cross-section of the ejectile particle with a target particle. This method relies on the previous two. 
This method works by placing a rectangle over the function to be integrated, generating a large number of points uniformly and randomly over the rectangle, and discarding those points 
above the function. By dividing the number of points left by the total number generated and multiplying by the rectangle area, the value of the integral is calculated.

4. 

Author: Jake Watson

Date started: 31/01/2016

Date finished: x

//refinements
pow inefficiency
division inefficiency for constants
profiling code - only works with lots of functions
integrate between 1 degree increments for 0-180 at the beginning of the code - rather for 130-140, 135-145
*/
...
int main() {
	...
	*/
	...
	energylab_file.open("Scattering angle and Scattering Energy - deuteron - LAB.txt", ios::out);
	for (int i = 0; i<proj_success; i++){
